...
#include <aie/bootstrap/GameTime.h>
...
namespace Debugging
{
	...
	void Debugger::Tick()
	{
		...
		{
			if (input->WasKeyPressed(EInputCodes::KeyF3))
			{
				m_debugModeEnabled = !m_debugModeEnabled;
				m_window->SetClearColor(m_debugModeEnabled ? Color(0, 0, 0, 0) : Color(255, 255, 255, 255));
			}
		}
#endif // _DEBUG
	...
	void Debugger::Render(Renderer* renderer)
	{
		ImGuiAdapter* imGuiAdapter = GetImGuiAdapter();
		...
		if (m_debugModeEnabled)
		{
			...
			if (ImGui::Begin("Debugger"))
			{
				...
				imGuiAdapter->EnumField("Verbosity", m_verbosity, { "Basic", "Advanced" });

				for (const auto& handler : m_handlers)
				{
					if (imGuiAdapter->BeginCollapsingGroup(handler->DebugCategory()))
					{
						handler->HandleEnabledCheckbox();

						imGuiAdapter->BeginDisabledGroup(!handler->IsDebuggingEnabled());

						handler->HandleImGui(m_verbosity);

						imGuiAdapter->EndDisabledGroup();
					}
					imGuiAdapter->EndCollapsingGroup();
				}
